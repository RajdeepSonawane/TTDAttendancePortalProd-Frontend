...
import { validationSchema } from "../Validations/checkOutPreviousValidation";
...
const CheckOutPreviousDashboard = () => {
    ...
    onSubmit:async (values,{setSubmitting,resetForm}) => {
      try {
        ...
        if (!values.checkOutTime) {
            throw new Error("Check-out time is missing");
          }
        const combinedDateTime = dayjs(`${values.checkOutDate} ${values.checkOutTime.format("HH:mm")}`).format("YYYY-MM-DDTHH:mm:ss");

        const payload = {
                checkOutLocation: values.checkOutLocation,
                checkOutOtherLocation: values.checkOutOtherLocation,
                activity: values.activity,
                checkOutDateTime: combinedDateTime
              };
        console.log(payload)
        const response = await axios.put(
          `${API_BASE_URL}/Attendance/check-out-previous`,
          payload,
          {
            headers: {
      ...
      sx={{
        ...
        backgroundColor: '#fff',      
         ...
         <TextField
            ...
            margin="normal"
            name="checkOutDate"
            label="Select Date to Check-Out"
            type="date"
          ...
          />
           ...
           <LocalizationProvider dateAdapter={AdapterDayjs}>
              <TimePicker
                label="Select Time"
                name='checkOutTime'
                value={formik.values.checkOutTime || null}
...
export default CheckOutPreviousDashboard;
